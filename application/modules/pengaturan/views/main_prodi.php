	...
	$(document).ready(function() {
	   table = $('#example').DataTable({
		...
		// Load data for the table's content from an Ajax source
			"ajax": {
			"url": "<?=site_url('pengaturan/prodi/prodi_list')?>/<?=$kd_fakultas?>",
			"type": "POST",
		},
...
<script>
	$(document).on('click','.ganti',function(){
					$('#tampil_form').load("<?=site_url()?>pengaturan/prodi/modal_form_prodi/<?=$kd_fakultas?>/"+ $(this).attr('id'),function(){
					$('#modal_form').modal('show');
					});
				});
	$(document).on('click','.tambah',function(){
					$('#tampil_form').load("<?=site_url()?>pengaturan/prodi/modal_form_prodi/<?=$kd_fakultas?>/",function(){
					$('#modal_form').modal('show');
					});
				});
	$(document).on('click','.hapus',function(){
		var cnf = confirm("Apakah anda yakin akan menghapus prodi ini?");
		if (cnf == true) {
			//alert ($(this).attr('id'));
			location.href = '<?=site_url()?>pengaturan/prodi/hapus_prodi/' + $(this).attr('id');
		}
	});
	$(document).on('click','.prodi',function(){
		location.href = '<?=site_url()?>pengaturan/prodi/index/' + $(this).attr('id');		
	});
</script>
